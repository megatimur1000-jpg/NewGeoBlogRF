...
// SONAR-AUTO-FIX (javascript:S1128): original: import HashtagChatServer from './hashtag-chat-server-integrated.js';
...
// SONAR-AUTO-FIX (javascript:S1128): original: import uploadRoutes from './src/routes/uploadRoutes.js';
...
// API для маркеров
// Удаляем дублирующий обработчик - теперь используется markerRoutes
// app.get('/api/markers', async (req, res) => {
//   try {
//     const result = await pool.query('SELECT * FROM map_markers ORDER BY created_at DESC');
...
app.get('/api/events', async (req, res) => {
  try {
    const result = await pool.query('SELECT *, is_user_modified, used_in_blogs FROM events ORDER BY date DESC');
    res.json(result.rows);
  } catch (error) {
    res.status(500).json({ error: 'Ошибка сервера' });
  }
...
// SONAR-AUTO-FIX (javascript:S1854): original: // SONAR-AUTO-FIX (javascript:S1481): original:         const chatWSS = new HashtagChatServer(server);
