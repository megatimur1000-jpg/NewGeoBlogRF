...
// SONAR-AUTO-FIX (javascript:S1128): original: import { authenticateToken } from '../middleware/auth.js';
  ...
  try {
    ...
    if (hasStatus) {
      if (isAdmin && requestedStatus) {
        // Админ может запросить конкретный статус - СТРОГАЯ фильтрация
...
router.post('/posts', optionalAuthenticateToken, async (req, res) => {
  ...
  console.log(`   Пользователь из токена: ${isGuest ? 'ГОСТЬ (нет токена или невалидный)' : author_id}`);
  console.log(`   Токен в заголовке: ${req.headers['authorization'] ? 'есть' : 'нет'}`);
  
  let userRole = isGuest ? 'guest' : 'registered';
  let isAdmin = false;
// SONAR-AUTO-FIX (javascript:S1854): original:   let finalStatus = 'pending'; // ПО УМОЛЧАНИЮ ВСЕГДА PENDING!
