...
import {
  ...
  approveContent,
  rejectContent,
// SONAR-AUTO-FIX (javascript:S1128): original:   hideContent,
  getModerationStats
...
router.post('/approve-local', async (req, res) => {
  try {
// SONAR-AUTO-FIX (javascript:S1854): original: // SONAR-AUTO-FIX (javascript:S1481): original:     const { content_type, local_id, content_data, author_id } = req.body;
    const userId = req.user?.id || req.user?.userId;
...
// SONAR-AUTO-FIX (javascript:S1481): original: // SONAR-AUTO-FIX (javascript:S1481): original:     let tableName, insertQuery, insertParams;
    let createdId;
