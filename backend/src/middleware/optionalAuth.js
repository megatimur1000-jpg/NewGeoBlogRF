...
export const optionalAuthenticateToken = (req, res, next) => {
  ...
  if (!decoded) {
    ...
    console.warn(`⚠️ Невалидный токен в запросе ${req.method} ${req.path}, обрабатываем как гостя`);
  ...
  if (decoded.exp && decoded.exp < Math.floor(Date.now() / 1000)) {
    console.warn(`⚠️ Истёкший токен в запросе ${req.method} ${req.path}, обрабатываем как гостя`);
    req.user = null;
    return next();
  }
