-- Улучшенный скрипт для удаления некачественных маркеров
-- Версия 2.0 - более точная фильтрация

-- Показываем количество маркеров до очистки
SELECT 'До очистки' as status, COUNT(*) as count FROM map_markers;

-- Показываем примеры проблемных маркеров
SELECT 'Примеры проблемных маркеров:' as info;
