...
export const MapWrapper = styled.div.attrs(() => ({
  ...
  /* Стили для временной метки */
  .temp-marker-icon {
    background-color: rgba(255, 0, 0, 0.6); /* Полупрозрачный красный */
    width: 25px; /* Немного больше, чтобы была заметнее */
    height: 25px;
    border-radius: 50%;
    border: 3px solid rgba(255, 255, 255, 0.8); /* Белый контур */
    box-shadow: 0 0 8px rgba(0, 0, 0, 0.5); /* Небольшая тень */
    cursor: pointer; /* Указываем, что метка кликабельна */
    display: flex;
    align-items: center;
    justify-content: center;
    font-size: 1.2em; /* Размер иконки, если добавляете */
