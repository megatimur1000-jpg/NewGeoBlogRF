  ...
  return (
    <div className="map-filters">
      ...
      <div className="accordion-section">
        <div
          className={`accordion-title${openSection === 'categories' ? ' open' : ''}`}
          onClick={() => setOpenSection(openSection === 'categories' ? '' : 'categories')}
        >
          <FaLeaf style={{ marginRight: 8 }} />
          Категории и хэштеги
          <span style={{ marginLeft: 'auto' }}>{openSection === 'categories' ? '▲' : '▼'}</span>
        </div>
      ...
      <div className="accordion-section">
        <div
          className={`accordion-title${openSection === 'radius' ? ' open' : ''}`}
          onClick={() => setOpenSection(openSection === 'radius' ? '' : 'radius')}
        >
          <FaCrosshairs style={{ marginRight: 8 }} />
          Радиус поиска
          <span style={{ marginLeft: 'auto' }}>{openSection === 'radius' ? '▲' : '▼'}</span>
        </div>
        {openSection === 'radius' && (
          <div className="accordion-content">
            <label>
            ...
            />
            <span style={{ minWidth: 40, textAlign: 'right', color: '#555' }}>{radius} км</span>
          </div>
        )}
      </div>
      ...
      <div className="accordion-section">
        <div
          className={`accordion-title${openSection === 'presets' ? ' open' : ''}`}
          onClick={() => setOpenSection(openSection === 'presets' ? '' : 'presets')}
        >
          <FaFire style={{ marginRight: 8 }} />
          Предустановленные сценарии
          <span style={{ marginLeft: 'auto' }}>{openSection === 'presets' ? '▲' : '▼'}</span>
        </div>
        {openSection === 'presets' && (
          <div className="accordion-content">
      ...
      {onLoadingModeToggle && (
        <div className="accordion-section">
          <div
            className={`accordion-title${openSection === 'loading' ? ' open' : ''}`}
            onClick={() => setOpenSection(openSection === 'loading' ? '' : 'loading')}
          >
            <FaMapIcon style={{ marginRight: 8 }} />
            Режим загрузки
            <span style={{ marginLeft: 'auto' }}>{openSection === 'loading' ? '▲' : '▼'}</span>
          </div>
          {openSection === 'loading' && (
            <div className="accordion-content">
      ...
      <div className="accordion-section">
        <div
          className={`accordion-title${openSection === 'map' ? ' open' : ''}`}
          onClick={() => setOpenSection(openSection === 'map' ? '' : 'map')}
        >
          <FaMapIcon style={{ marginRight: 8 }} />
          Настройки карты
          <span style={{ marginLeft: 'auto' }}>{openSection === 'map' ? '▲' : '▼'}</span>
        </div>
        {openSection === 'map' && (
          <div className="accordion-content">
      ...
      <div className="map-filters-footer">
        {onShowAllMarkers && (
          <div className="show-all-markers-checkbox">
            <label>
              <input
                type="checkbox"
        ...
        <div className="action-buttons">
          <button className="map-filters-btn secondary" type="button" onClick={onReset}>Сбросить</button>
          <button className="map-filters-btn primary" type="button" onClick={onApply}>Применить</button>
        </div>
      </div>
    </div>
  );
